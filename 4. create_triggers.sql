...
DECLARE @SAMPLE_TIME as datetime;
SET @SAMPLE_TIME = (SELECT sample_time FROM performance_sample where id = (SELECT sample_id FROM INSERTED));
...
worst_disk_usage,p.worst_disk_busy,p.used_swap_percent, @SAMPLE_TIME
FROM inserted p
GO
...
CREATE TRIGGER trig_insert_disk_total
ON performance_disk_total
INSTEAD OF INSERT
AS
...
INSERT INTO performance_disk_total
           ([pct_time_busy]
           ,[avg_queue_req]
           ,[rw_sec]
           ,[blocks_sec]
...
CREATE TRIGGER trig_insert_lpar_workload
